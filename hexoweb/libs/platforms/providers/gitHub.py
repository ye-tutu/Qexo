...
class Github(Provider):
    ...
    def get_post(self, post):
        try:
            content = self.repo.get_contents(
                f"{self.path}source/_drafts/{post}", self.branch
            ).decoded_content.decode("utf8")

            print(f"从草稿中获取文章{post}成功")
        except:
            content = self.repo.get_contents(
                f"{self.path}source/_posts/{post}", self.branch
            ).decoded_content.decode("utf8")

            print(f"获取文章{post}成功")
        return content

    def get_content(self, file):
        print(f"获取文件{file}")
        return self.repo.get_contents(
            self.path + file, self.branch
        ).decoded_content.decode("utf8")
    ...
    def get_posts(self, _path=""):
        ...
        names = []
        try:
            drafts = self.repo.get_contents(
                f'{self.path}source/_drafts{_path}', ref=self.branch
            )
            ...
            for i in range(len(drafts)):
                if drafts[i].type == "file" and drafts[i].name[-2:] == "md":
                    _drafts.append(
                        {
                            "name": drafts[i].path.split("source/_drafts/")[1][
                                :-3
                            ],
            ...
            pass
        try:
            posts = self.repo.get_contents(
                f'{self.path}source/_posts{_path}', ref=self.branch
            )

            for i in range(len(posts)):
                if (
                    posts[i].type == "file"
                    and posts[i].name[-2:] == "md"
                    and posts[i].path.split("source/_posts/")[1] not in names
                ):
                    _posts.append(
                        {
                            "name": posts[i].path.split("source/_posts/")[1][:-3],
    ...
    def get_pages(self):
        posts = self.repo.get_contents(f'{self.path}source', ref=self.branch)
        results = []
        for post in posts:
            if post.type == "dir":
                for i in self.repo.get_contents(
                        self.path + post.path,
                        ref=self.branch):
                    if i.type == "file" and i.name in ["index.md", "index.html"]:
    ...
    def get_configs(self):
        ...
        try:
            themes = self.repo.get_contents(f"{self.path}themes", ref=self.branch)
            for theme in themes:
                if theme.type == "dir":
            ...
            pass
        # 检索 source 仅最多一层目录
        sources = self.repo.get_contents(f"{self.path}source", ref=self.branch)
        for source in sources:
            if source.type == "file":
                try:
                    if source.name[-4:] in [".yml", "yaml"]:
